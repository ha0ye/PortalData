#Functions for retrievingss and processing NDVI data for Portal.
...
##################################################
get_gimms_download_list <- function(gimms_folder = './GIMMS', version = 0)
{
...
{
  files <- list.files(gimms_folder, pattern = "^geo[0-9]{2}")
  #hdr files are created from some of the gimms processing that we don't want to
  #use here.
  gimms_files <- file.path(gimms_folder, files)
...
lat = 31.937769)
  ...
  out <- do.call(rbind, 
lapply(gimms_files, extract_gimms_data, 
site = site))
  return(out)
}
